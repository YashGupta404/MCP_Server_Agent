// Way 2: make Entity Name (232) user-entered on upload for COM - Application (account/opportunity/campaign),
// and add hfs_sObject_Id (223) as the record-scoping field. Reliable Node MCP client (JSON + SSE).
const fs = require('fs');
...
async function callTool(name, args) {
  const init = await rpc('initialize', { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'way2', version: '0.1' } });
  const sid = init.sid;
...
(async () => {
  ...
  try {
    ...
    for (const e of boc.additionalConfig) {
      if (e.ecmContentTypeName === 'COM - Application' && targets.includes(e.busObject)) {
        ...
        const f232 = mm.find(m => m.ecmFieldName === '232');
        if (f232) f232.inputSource = '6';
    ...
    out.push('changed entries: ' + changed);
  ...
  fs.writeFileSync(process.env.TEMP + '/way2.txt', out.join('\n'));
  console.log('done');
})();
